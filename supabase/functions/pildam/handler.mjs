...
export function createHandler(env,fetcher=fetch){
  ...
  async function classSettings(){const rows=await db('pildam_class_settings?select=student_code,accuracy_weight&id=eq.1');return rows?.[0]||{student_code:null,accuracy_weight:75};}
  async function handle(req){const path=new URL(req.url).pathname.split('/').pop(),now=Date.now(),a=await auth(req);
    if(!signingKey||!env.SUPABASE_SERVICE_ROLE_KEY)return json({error:'선생님이 수업 설정을 준비하고 있어요.'},503);
    ...
    if(path==='assignment'&&req.method==='GET'){if(!a)return json({error:'수업 코드를 다시 입력해주세요.'},401);const s=await classSettings();return json({assignment:await assignment(),accuracyWeight:s.accuracy_weight});}
    if(path==='settings'&&req.method==='GET'){if(!a||a.role!=='teacher')return json({error:'선생님용 코드로 다시 들어와주세요.'},401);const s=await classSettings();return json({hasStudentCode:!!s.student_code,accuracyWeight:s.accuracy_weight});}
    if(req.method!=='POST')return json({error:'찾을 수 없는 요청입니다.'},404);
    if(path==='login'||path==='teacher-login'){const teacher=path==='teacher-login',s=await classSettings(),code=teacher?env.TEACHER_CODE:(s.student_code||env.CLASS_CODE);if(!code||code.length<(teacher?8:4))return json({error:teacher?'선생님용 코드를 먼저 설정해주세요.':'학생용 수업 코드를 먼저 설정해주세요.'},503);if(!await limited('login:'+Math.floor(now/600000),120,now+1200000))return json({error:'입력 시도가 많아요. 잠시 후 다시 시도해주세요.'},429);const data=await body(req);if(typeof data.code!=='string'||!equal(await digest(data.code),await digest(code)))return json({error:'코드를 확인해주세요.'},401);return json({ok:true,session:await issue(teacher?'teacher':'student')});}
    ...
    if(path==='settings'){if(!a||a.role!=='teacher'||!equal(req.headers.get('X-App-Token'),a.csrf))return json({error:'선생님용 코드로 다시 들어와주세요.'},401);const data=await body(req);const next={id:1,accuracy_weight:Number(data.accuracyWeight),updated_at:new Date().toISOString()};if(!Number.isInteger(next.accuracy_weight)||next.accuracy_weight<50||next.accuracy_weight>90)return json({error:'문장 일치도 비율은 50%에서 90% 사이로 정해주세요.'},400);if(data.studentCode!==undefined){if(typeof data.studentCode!=='string'||data.studentCode.length<4||data.studentCode.length>32)return json({error:'학생용 수업 코드는 4자에서 32자 사이로 입력해주세요.'},400);next.student_code=data.studentCode;}await db('pildam_class_settings?id=eq.1',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify(next)});const s=await classSettings();return json({hasStudentCode:!!s.student_code,accuracyWeight:s.accuracy_weight});}
